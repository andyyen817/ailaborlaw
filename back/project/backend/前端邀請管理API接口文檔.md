...
class InviteLeaderboard {
  ...
  async loadLeaderboard(limit = 10, startDate = null, endDate = null) {
    try {
      let url = `http://localhost:7070/api/v1/invites/leaderboard?limit=${limit}`;
      if (startDate) url += `&startDate=${startDate}`;
      if (endDate) url += `&endDate=${endDate}`;
      ...
      <div class="leaderboard">
        ...
        <div class="my-ranking">
          ...
          <div class="ranking-card">
            ...
            邀請 ${data.myRanking.totalInvited} 人 - 
            獲得 ${data.myRanking.totalBonus} 次獎勵
          </div>
        </div>
        ...
        <div class="leaderboard-list">
          ${data.leaderboard.map(user => `
            <div class="leaderboard-item ${user.rank <= 3 ? 'top-three' : ''}">
              <span class="rank">${this.getRankDisplay(user.rank)}</span>
              <span class="name">${user.userName}</span>
              <span class="stats">
                <span class="invited">${user.totalInvited}人</span>
                <span class="bonus">${user.totalBonus}次</span>
              </span>
            </div>
          `).join('')}
        </div>
  ...
  bindEvents() {
    this.container.addEventListener('change', (e) => {
      if (e.target.id === 'limitSelect') {
        this.currentLimit = parseInt(e.target.value);
        this.loadLeaderboard(this.currentLimit);
      }
    });
  }
}
